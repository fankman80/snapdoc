                    ...
                    foreach (var plan in GlobalJson.Data.Plans)
                    {
                        if (GlobalJson.Data.Plans[plan.Key].Pins != null)
                        {
                            foreach (var pin in GlobalJson.Data.Plans[plan.Key].Pins)
                            {
                                ...
                                {
                                    switch (columnList.FirstOrDefault(item => item.Item1 == column).Item2)
                                    {
                                        case "${pin_nr}":
                                            newRow.Append(new D.TableCell(new D.Paragraph(new D.Run(new D.Text(i.ToString())))));
                                            break;

                                        case "${pin_planName}":
                                            newRow.Append(new D.TableCell(new D.Paragraph(new D.Run(new D.Text(GlobalJson.Data.Plans[plan.Key].Name)))));
                                            break;
                                        ...
                                        case "${pin_posImage}":
                                            if (SettingsService.Instance.IsPosImageExport)
                                            {
                                                ...
                                                newRow.Append(new D.TableCell(new D.Paragraph(new D.Run(_imgPlan))));
                                            }
                                            break;
                                        ...
                                        case "${pin_fotoList}":
                                            if (SettingsService.Instance.IsImageExport)
                                            {
                                                ...
                                                newRow.Append(new D.TableCell(paragraph));
                                            }
                                            break;
