...
public partial class CameraView : ContentPage
{
    ...
    protected override void OnSizeAllocated(double width, double height)
    {
        ...
        if (_isStarted)
        {
            // Rotation / Groessenaenderung nach dem Start -> Preview neu aufbauen.
    ...
    private async Task InitializeCameraSelection()
    {
        ...
        {
            // Notnagel: hoechste verfuegbare Aufloesung, niemals Size(0,0) uebergeben.
    ...
    /// abweichende Seitenverhaeltnis zu einem sichtbaren Zoom/Crop fuehren. Der
    /// gewuenschte Zuschnitt erfolgt stattdessen beim Speichern (CropToRatio).
    /// iOS/Windows: die uebergebene Size gilt auch fuer die Preview, hier darf das
    /// Zielverhaeltnis direkt gewaehlt werden.
    /// </summary>
    ...
    /// <summary>
    /// Das Seitenverhaeltnis, das die Preview tatsaechlich zeigt.
    /// Unter Android ist das immer das native Sensorformat, nicht das gewaehlte.
    /// </summary>
    ...
    /// <summary>
    /// Dimensioniert den Preview-Container exakt auf das Verhaeltnis, das die
    /// Preview liefert. So wird nichts mehr center-cropped.
    /// </summary>
    private void UpdateCameraLayout(double width, double height)
    {
        ...
        double cameraRatio = GetPreviewRatio();
    ...
    private void PopulateRatioButtons()
    {
        ...
        // Wichtig: gleiche Normalisierung wie in GetOptimalSize, sonst passen die
        // Buttons nicht zu _userSelectedRatio (Portrait-Aufloesungen ergaeben < 1.0).
    ...
    /// <summary>
    /// Setzt den Slider neu auf. Das Zuweisen von Minimum/Maximum loest
    /// ValueChanged aus, deshalb werden die Events waehrenddessen unterdrueckt -
    /// sonst wird ein ungewollter ZoomFactor in die Kamera geschrieben.
    /// </summary>
    private void ConfigureZoom()
    {
        ...
        try
        {
            ...
            // Nach jedem Start definiert auf 1x zuruecksetzen.
    ...
    private async void OnCaptureClicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (stream != null)
            {
                _tempFilePath = await SavePhotoToCache(stream, _userSelectedRatio);
    ...
    private static async Task<string?> SavePhotoToCache(Stream photoStream, double userRatio)
    {
        ...
        // maximale Pixelzahl). Der vom Benutzer gewaehlte Zuschnitt wird hier
        // verlustfrei in der Pixelebene nachgezogen.
        await Task.Run(() => CropToRatio(path, userRatio));
#endif
        return path;
    }
    ...
    /// userRatio == -1.0 bedeutet "Bildschirmformat" und wird hier uebersprungen,
    /// -1.0 wird bereits beim Start beruecksichtigt.
    /// </summary>
    private static void CropToRatio(string path, double userRatio)
    {
        try
        {
            if (userRatio <= 0) return;

            using var original = SKBitmap.Decode(path);
            if (original == null) return;

            double target = userRatio < 1.0 ? 1.0 / userRatio : userRatio;
            ...
            if (Math.Abs(currentRatio - target) < 0.02) return; // passt bereits

            int newLong, newShort;
            if (currentRatio > target)
            {
                newShort = (int)shortSide;
                newLong = (int)Math.Round(shortSide * target);
            }
            else
            {
                newLong = (int)longSide;
                newShort = (int)Math.Round(longSide / target);
            }
...
#if ANDROID
        // Android behaelt das native Sensorformat; nur der Zuschnitt aendert sich.
        UpdateCameraLayout(Width, Height);
